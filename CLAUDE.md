...
```
Siehe Kommentar am Kopf des Skripts für die genauen Aufrufe der drei aktuellen Kämpfer (Cassius/Brannok/Solkan).

**Deployment:** Läuft live über GitHub Pages (`main`-Branch, Root-Verzeichnis, kein Build/CI nötig — reine statische Dateien) unter `https://carlomeier1981.github.io/game-mortalkombat/`. Das Repo muss dafür öffentlich sein (kostenlose GitHub-Pages-Stufe unterstützt keine privaten Repos). Ein `git push` auf `main` reicht zum Deployen; GitHub baut automatisch neu (dauert i. d. R. 1–5 Minuten). Alle Asset-/Modul-Pfade im Projekt sind bewusst relativ gehalten, damit die Seite auch unter dem GitHub-Pages-Unterpfad (`/game-mortalkombat/` statt Domain-Root) korrekt funktioniert.
...
- **PWA**: `manifest.json` + `service-worker.js` (Cache-first mit Hintergrund-Refresh, App-Shell-Precache) + `icons/` machen das Spiel auf Mobilgeräten über "Zum Startbildschirm hinzufügen" installierbar. `CACHE_NAME` in `service-worker.js` bei Änderungen an gecachten Dateien hochzählen, sonst bekommen bereits installierte Clients die alte Version (bzw. bekommen sie ohnehin erst nach 1–2 Reloads/Neustarts, weil "Cache-first" bewusst zuerst die alte Version zeigt und im Hintergrund aktualisiert). Service-Worker-Registrierung in `js/main.js` ist best-effort (Fehler werden verschluckt) und ließ sich in der Claude-Browser-Testumgebung nicht verifizieren (eigener Chromium-Build, `register()` schlägt dort konsequent mit "unknown error fetching script" fehl, obwohl das File selbst per normalem `fetch()` einwandfrei lädt) — Manifest/Icons/Installierbarkeit dagegen schon.
- **Touch-Controls-Gotcha**: Die "Touch-Buttons auf Desktop ausblenden"-Regel in `css/style.css` nutzt bewusst `@media (hover: hover) and (pointer: fine)`, **nicht** eine `min-width`-Breakpoint. Ein breiten-basierter Breakpoint schlägt fehl, weil Tablets im Querformat oft eine CSS-Viewport-Breite >900px melden und damit fälschlich als Desktop erkannt würden (live auf einem Samsung-Tablet aufgetreten und so gefixt).
- **Asset-Ordner**: `backgrounds/` (Arena-Fotos, im Spiel geladen), `models/` (kompilierte GLBs, im Spiel geladen), `icons/` (PWA-App-Icons, prozedural per Canvas erzeugt statt von Hand gemalt), `avatar-templates/` (rohe FBX-Quellpacks für die Skeletal-Kämpfer). `avatar-pics/` enthält Referenzbilder (u. a. für den inzwischen gelöschten Kage, plus mindestens ein noch ungenutztes Konzeptbild) — reines Design-Rohmaterial, nicht laufzeitrelevant. Es gibt kein `.gitignore` — `avatar-templates/` wurde bisher bewusst nie mit `git add` erfasst (reines lokales Rohmaterial, nicht laufzeitrelevant), taucht also als untracked in `git status` auf.
